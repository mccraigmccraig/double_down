...
DoubleDown is a test-double library for Elixir — mocks and fakes — with
multiple routes to add testing boundaries to your system. It goes beyond
mocks with stateful test doubles (fakes), including an `Ecto.Repo` fake
powerful enough to use ExMachina factories without a database.
...
that use the database merely as a slow way of getting data into the right
place can run without one. DoubleDown's in-memory Repo lets you keep the
factory, drop the DB, and run your tests `async: true`.
