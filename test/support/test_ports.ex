# Port facades for test contracts.
# These are in a separate file from the contracts so that the contracts
# are fully compiled before the Port modules try to read __port_operations__/0.

defmodule HexPort.Test.Greeter.Port do
  use HexPort.Facade, contract: HexPort.Test.Greeter, otp_app: :hex_port
end

# Facade for HexPort.Repo.Contract (library-provided contract).
# In a real app this would be defined in the application's namespace,
# e.g. MyApp.Repo. Here we use HexPort.Repo.Port for test purposes.
defmodule HexPort.Repo.Port do
  use HexPort.Facade, contract: HexPort.Repo.Contract, otp_app: :hex_port
end

defmodule HexPort.Test.Counter.Port do
  use HexPort.Facade, contract: HexPort.Test.Counter, otp_app: :hex_port
end

defmodule HexPort.Test.BangVariants.Port do
  use HexPort.Facade, contract: HexPort.Test.BangVariants, otp_app: :hex_port
end

defmodule HexPort.Test.ZeroArg.Port do
  use HexPort.Facade, contract: HexPort.Test.ZeroArg, otp_app: :hex_port
end

defmodule HexPort.Test.Documented.Port do
  use HexPort.Facade, contract: HexPort.Test.Documented, otp_app: :hex_port
end

defmodule HexPort.Test.MultiParam.Port do
  use HexPort.Facade, contract: HexPort.Test.MultiParam, otp_app: :hex_port
end

defmodule HexPort.Test.AliasedTypes.Port do
  use HexPort.Facade, contract: HexPort.Test.AliasedTypes, otp_app: :hex_port
end
