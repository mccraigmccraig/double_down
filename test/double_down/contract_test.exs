defmodule DoubleDown.ContractTest do
  ...
  describe "bang variant generation" do
    setup do
      DoubleDown.Testing.set_fn_handler(DoubleDown.Test.BangVariants, fn
    ...
    test "auto-detected bang: generates bang for {:ok, T} | {:error, T} return type" do
      {:module, _} = Code.ensure_loaded(DoubleDown.Test.BangVariants.Port)
      assert function_exported?(DoubleDown.Test.BangVariants.Port, :auto_bang!, 1)
      assert "auto-test" = DoubleDown.Test.BangVariants.Port.auto_bang!("test")
    end
    ...
    test "auto-detected bang: raises on error" do
      ...
      assert_raise RuntimeError, ~r/auto_bang failed/, fn ->
        DoubleDown.Test.BangVariants.Port.auto_bang!("test")
      end
    end

    test "forced bang: generates bang even without {:ok, T} return type" do
      {:module, _} = Code.ensure_loaded(DoubleDown.Test.BangVariants.Port)
      assert function_exported?(DoubleDown.Test.BangVariants.Port, :forced_bang!, 1)
    end
    ...
    test "forced bang: unwraps {:ok, value}" do
      ...
      assert "wrapped-test" = DoubleDown.Test.BangVariants.Port.forced_bang!("test")
    end
    ...
    test "forced bang: raises on {:error, reason}" do
      ...
      assert_raise RuntimeError, ~r/forced_bang failed/, fn ->
        DoubleDown.Test.BangVariants.Port.forced_bang!("test")
      end
    end

    test "suppressed bang: does not generate a bang variant" do
      {:module, _} = Code.ensure_loaded(DoubleDown.Test.BangVariants.Port)
      refute function_exported?(DoubleDown.Test.BangVariants.Port, :suppressed_bang!, 1)
    end

    test "custom bang: generates bang with custom unwrap function" do
      {:module, _} = Code.ensure_loaded(DoubleDown.Test.BangVariants.Port)
      assert function_exported?(DoubleDown.Test.BangVariants.Port, :custom_bang!, 1)

      # custom_bang returns "custom-test", custom unwrap maps non-nil to {:ok, value}
      assert "custom-test" = DoubleDown.Test.BangVariants.Port.custom_bang!("test")
    end
    ...
    test "custom bang: raises when custom unwrap returns {:error, reason}" do
      ...
      assert_raise RuntimeError, ~r/custom_bang failed/, fn ->
        DoubleDown.Test.BangVariants.Port.custom_bang!("test")
      end
    end

    test "no bang: plain return type without bang option gets no bang variant" do
      {:module, _} = Code.ensure_loaded(DoubleDown.Test.BangVariants.Port)
      refute function_exported?(DoubleDown.Test.BangVariants.Port, :no_bang!, 1)
    end
    ...
    test "Greeter auto-detects bang for fetch_greeting but not greet" do
      ...
      assert function_exported?(DoubleDown.Test.Greeter.Port, :fetch_greeting!, 1)
      refute function_exported?(DoubleDown.Test.Greeter.Port, :greet!, 1)
    end
    ...
    test "bang variants call the non-bang function (no double dispatch)" do
      ...
      DoubleDown.Testing.enable_log(DoubleDown.Test.Greeter)

      # Call bang variant
      assert "Hello, test!" = DoubleDown.Test.Greeter.Port.fetch_greeting!("test")

      # Should only see one dispatch (the non-bang call), not two
      log = DoubleDown.Testing.get_log(DoubleDown.Test.Greeter)
      assert length(log) == 1
      assert [{DoubleDown.Test.Greeter, :fetch_greeting, ["test"], {:ok, "Hello, test!"}}] = log
    end

    test "ZeroArg auto-detects bang for get_version but not health_check" do
      {:module, _} = Code.ensure_loaded(DoubleDown.Test.ZeroArg.Port)
      assert function_exported?(DoubleDown.Test.ZeroArg.Port, :get_version!, 0)
      refute function_exported?(DoubleDown.Test.ZeroArg.Port, :health_check!, 0)
    end
  end
